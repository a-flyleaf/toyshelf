...
info: |-
  ...
  - He and [Tara]({%include url.html%}/disaster-crew/tara) used to prank call the scam lines and talk local delivery services into "updog"s. Nowadays he kills time by trying to get the kids in <i>Call of Duty</i> to stop screaming profanities—when, y'know, he's not yelling back.
  - Once wanted to be a lifeguard. The first time he "saved" someone, they both nearly drowned.
...
design: |-
  ...
  - In 2012, usually had an obvious tan. Fairly athletic.
...
trivia: |-
  - Originally named "Nathaniel '[Nate](https://bulbapedia.bulbagarden.net/wiki/Nate)' Pine(s)"; don't think I ever figured out whether the surname had an <i>S</i> at the end or not. The name still sounds fitting enough, but it got changed when Nico stopped being a pokémon person.
---
<div id="gallery">{%for art in site.art%}{%if art.tags contains "nico"%}<a href="{{art.url}}"><img src="{%include url.html%}/assets/img/art/{{art.date|date:"%F"}}-tn-{{page.nm}}.jpg" alt="{{art.title}}"/></a>{%endif%}{%endfor%}</div>